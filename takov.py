...
tahap6 = raw_input(" %s6 )%s Salin link URL hasil kombinasi tsb, lalu kirim link URL nya ke%s ADMIN%s Group tsb%s [ENTER]%s" % (W, G, C, G, R, N))
print "%s-------------------%s" % (R, N)

tahap7 = raw_input(" %s7 )%s Selanjutnya tinggal menunggu hingga link URL nya diklik oleh si %sADMIN%s Group tsb%s [ENTER]%s" % (W, G, C, G, R, N))
print "%s-------------------%s" % (R, N)

tahap8 = raw_input(" %s8 )%s Kalau sudah ada pemberitahuan bahwa kamu sudah dilantik menjadi%s ADMIN%s, maka kamu harus secepatnya menendang semua %sADMIN%s yg lain di Group tsb, sebelum mereka sadar akan hal ini...%s [ENTER]%s" % (W, G, C, G, C, G, R, N))
print "%s-------------------%s" % (R, N)

tahap9 = raw_input(" %s9 )%s Good Luck%s [ENTER]%s" % (W, G, R, N))
print (" ") 

time.sleep(3)
print colored('ADMIN yg pintar akan langsung menyadari Link URL apa yg anda kirimkan padanya, jadi saya sarankan menggunakan Shortlink untuk memyamarkan Link URL tsb. Bisa dengan goo.gl atau bit.ly', 'green') 
time.sleep(2)
print colored('Terima Kasih. Sudah bersedia menggunakan Tool ini', 'green') 
time.sleep(2)
